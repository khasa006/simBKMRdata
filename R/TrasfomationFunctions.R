...
#'
#' @description This script provides transformation functions for data scaling and normalization.
#'
#' @export
...
#' @param method Character string specifying the method: "sd" (standard deviation) or "mad" (median absolute deviation).
#' @return A numeric vector where values are divided by the chosen method's statistic.
#' @export
#'
#' @examples
...
#'
#' @param x A numeric vector or column of a dataframe to be transformed.
#' @param fracRoot The fractional power to which each element in `x` should be raised. Defaults to 0.5 (square root).
...
#' @param shift A numeric value added to `x` before applying the logarithm to avoid log(0). Defaults to 1.
#' @return A numeric vector of transformed values.
#' @export
#'
#' @examples
