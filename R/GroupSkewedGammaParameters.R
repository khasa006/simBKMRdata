...
#' @return A list of statistics (sample size, mean, standard deviation, correlation matrix, skewness, alpha, beta)
#' @export
.group_stats <- function(x_df) {
  ...
  # Calculate Gamma distribution parameters (shape and rate)
  alpha_beta <- data.frame(
    alpha = (mean_vector^2) / samp_sd^2,  # Shape parameter
    beta = samp_sd^2 / mean_vector        # Rate parameter
  )
