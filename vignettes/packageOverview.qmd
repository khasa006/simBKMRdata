---
title: "simBRKRdata:Helper Functions for Bayesian Kernel Machine Regression"
...
This paper presents an R package for simulating multivariate environmental exposure data and supporting feature selection in Bayesian Kernel Machine Regression (BKMR). The software generates correlated data from multivariate normal and skewed Gamma distributions, allowing users to characterize realistic non-normal exposure patterns. It calculates key multivariate distributional properties, including mean, variance, skewness, shape and rate parameters, and correlation matrices. The package also estimates Posterior Inclusion Probability thresholds for BKMR feature selection under non-normality. Its functionality is demonstrated using adolescent environmental exposure data to examine associations between metal mixtures and IQ, providing a reusable tool for environmental health and mixture research.
...
A dynamic threshold function was derived from the logistic model to adjust thresholds based on CV and sample size[@tanvirhasan2025].
