...
In environmental health studies, the relationship between multiple environmental exposures and health outcomes, such as cognitive development, is often investigated using complex datasets that exhibit non-normality. This paper introduces an R package designed to simulate multivariate environmental exposure data and estimate feature selection thresholds to aid in research to apply and extend the Bayesian Kernel Machine Regression (BKMR) methodology. The package enables researchers to generate data from multivariate normal and multivariate skewed Gamma distributions, common in environmental exposure research, and calculate multivariate statistical features such as mean, variance, skewness, shape, and rate vectors, and correlation matrices. This package also facilitates the calculation of a Posterior Inclusion Probability (PIP) threshold for feature selection in BKMR, offering an approach that accounts for non-normal data (based on the work in [CITE]). The effectiveness of the package is demonstrated through a real-world application using data from an adolescent environmental exposure study, where metal exposures are related to IQ scores.
...
We began by examining the real-world environmental exposure dataset metalExposChildren_df, which includes measurements of various metal concentrations—Cadmium, Mercury, Arsenic, Lead, and Manganese—in children, along with their corresponding IQ scores. After preprocessing to remove missing values, we visualized the distribution of each metal concentration. The histograms and density plots (Figure 1) revealed moderate to high skewness in several metals, particularly Cadmium and Arsenic, which deviated substantially from normality.
...
To simulate synthetic data reflecting real-world distributions, we estimated key statistical parameters for each sex group (Male and Female) using the calculate_stats_gamma() function, employing the method of moments. This function returned group-specific parameter lists, including sample size, mean vectors, Gamma distribution shape and rate parameters, and Spearman correlation matrices. These group-level parameters provided the basis for generating realistic skewed multivariate distributions of environmental exposures.
...
We applied BKMR to evaluate the associations between environmental exposures and children’s IQ scores. Prior to analysis, each metal variable was log-transformed using the trans_log() function to reduce skewness. The exposure matrix consisted of the five log-transformed metals, and the outcome variable was the continuous IQ score.

Using the kmbayes() function from the bkmr package, we fit the model with 2000 MCMC iterations, enabling variable selection through posterior inclusion probabilities (PIPs). Posterior inference indicated that several metals had elevated PIP values, suggesting strong associations with IQ. The PIPs were extracted using ExtractPIPs() and sorted in descending order.

Notably, Arsenic and Lead emerged as likely key contributors to cognitive development outcomes. However, determining which PIP values were statistically meaningful required the use of a calibrated, data-driven threshold.
...
By comparing each metal’s PIP against the calculated threshold, we identified a refined subset of significant exposures. This approach allows for a statistically principled method of feature selection within the BKMR framework, ensuring robust and interpretable results even when the data deviate from normality.
